...
DSTATUS disk_initialize ( __attribute__((unused))	BYTE pdrv ) {
	return ( sd1.initialize() != EC_MICRO_SD_TYPE::ERROR ) ? 0 : STA_NOINIT;
}

DSTATUS disk_status ( __attribute__((unused)) BYTE pdrv ) {
	return ( DSTATUS )sd1.getStatus();
}
...
DRESULT disk_read (	__attribute__((unused))	BYTE pdrv,
	...
	do {
		funcR = ( DRESULT )sd1.readSector( sector, buff, count, 1000 );
		l--;
		USER_OS_DELAY_MS(1);
...
DRESULT disk_write ( __attribute__((unused))	BYTE pdrv,
	...
	do {
		funcR = ( DRESULT )sd1.writeSector( buff, sector, count, 1000 );
		l--;
		USER_OS_DELAY_MS(1);
	} while ( ( funcR != RES_OK ) && ( l != 0 ) );
...
DRESULT disk_ioctl ( BYTE pdrv, BYTE cmd, void* buff ) {
	...
	case GET_SECTOR_COUNT :
		if ( sd1.getSectorCount( *( ( uint32_t* )buff ) ) == EC_SD_RESULT::OK )
			res = DRESULT::RES_OK;
		break;

	case GET_SECTOR_SIZE :
		if ( sd1.getBlockSize( *( ( uint32_t* )buff ) ) == EC_SD_RESULT::OK )
			res = DRESULT::RES_OK;
		break;
